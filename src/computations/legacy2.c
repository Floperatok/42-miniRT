...
/*   Updated: 2024/02/20 23:02:28 by nsalles          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point	set_ray(t_point pixel, t_camera cam)
{
	t_point	ray;

	ray.x = (pixel.x - HALF_SCREEN_W) / HALF_SCREEN_W * tan(cam.fov / 2 * RAD);
	ray.y = (pixel.y - HALF_SCREEN_H) / HALF_SCREEN_H * tan(cam.fov / 2 * RAD);
	ray.z = 1;

	// ray.y = (ray.y * cos(cam.direction->x)) - (ray.z * sin(cam.direction->x));
	// ray.z = (ray.y * sin(cam.direction->x)) + (ray.z * cos(cam.direction->x));
	// ray.x = (ray.z * sin(cam.direction->y)) + (ray.x * cos(cam.direction->y));
	// ray.z = (ray.z * cos(cam.direction->y)) - (ray.x * sin(cam.direction->y));
	// ray.x = (ray.x * cos(cam.direction->x)) - (ray.y * sin(cam.direction->x));
	// ray.y = (ray.x * sin(cam.direction->x)) + (ray.y * cos(cam.direction->x));
	return (ray);
}
